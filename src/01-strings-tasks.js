...
function getRectangleString(width, height) {
  ...
  const MIDDLE = '│ │\n';
  const BOTTOM = '└ ┘\n';
  return `${TOP.replace(' ', '─'.repeat(width - 2))}${MIDDLE.replace(' ', ' '.repeat(width - 2)).repeat(height - 2)}${BOTTOM.replace(' ', '─'.repeat(width - 2))}`;
}
...
function encodeToRot13(str) {
  ...
  const handle = (x) => (START.indexOf(x) > -1 ? END[START.indexOf(x)] : x);
