...
function isTriangle(a, b, c) {
  const maxEl = Math.max(a, b, c);
  const arr = [a, b, c];
  const indexOfMax = arr.indexOf(maxEl);
  arr.splice(indexOfMax, 1);
  return maxEl < arr.reduce((prev, cur) => prev + cur);
}
...
function isCreditCardNumber(ccn) {
  return ((ccn.toString().split('')
    ...
    .reduce((acc, x, idx) => {
      if (idx % 2) {
        const temp = parseInt(x, 10) * 2;
        if (temp > 9) return acc + temp - 9;
        return acc + temp;
      }
